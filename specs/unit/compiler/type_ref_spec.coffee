...
describe "Compiler#typeRef", ->
  ...
  it 'creates a proxy if the type exists', ->
    should(compiler.typeRef('Date')).be.an.instanceof(TypeRef)

  it 'creates a proxy if type does not exists', ->
    should(compiler.typeRef('nosuchone')).be.an.instanceof(TypeRef)

  it 'supports denoting types through information contracts', ->
    should(compiler.typeRef('Date/timestamp')).be.an.instanceof(TypeRef)
